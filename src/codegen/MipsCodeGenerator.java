...
public class MipsCodeGenerator {
    ...
    */
    ...
    boolean checkIsFloat(String a) {
        ...
        else if (staticFloatList.contains(a)) {
            return true;
        }
    ...
    private void handleReturn(Return instruction) {
        ...
        // function with non-void return value
            ...
            if (locToType.get(returnValueIndex + "(" + STACK_POINTER + ")") == RegisterType.Float) {
                emit(new lw(FUNCTION_FLOAT_RETURN_VALUE_0, returnValueIndex + "(" + STACK_POINTER + ")"));
            }
            else {
            ...
            if (checkIsFloat(returnValue)) {
                emit(new li(FUNCTION_FLOAT_RETURN_VALUE_0, returnValue));
            }
            else {
    ...
    private void handleCompOp(String a, String b, String label, String op) {
        ...
        if (varToRegister.containsKey(b)) {
            ...
            addBackB = true;
            register_b = getRegister(false, addrIsFloat3(b));
        }
        emit(getLoadCommand(register_b, b));
    ...
    private void handleAssignArray(AssignArray instruction) {
        if (isFloatInstrArray3(instruction)) {
            handleFloatAssignArray(instruction);
            return;
        }
    ...
    private void handleArrayLoad(Array_load instruction) {
        if (isFloatInstrArray(instruction)) {
            handleFloatArrayLoad(instruction);
            return;
        }
    ...
    private void handleArrayStore(Array_store instruction) {
        if (isFloatInstrArray2(instruction)) {
            handleFloatArrayStore(instruction);
            return;
        }
        String arrayVar = instruction.args().get(0);
    ...
    private void handleFloatAssignArray(AssignArray instruction) {
        String arrayVar = instruction.args().get(0);
        int size = Integer.parseInt(instruction.args().get(1));
        String defaultValue = instruction.args().get(2);
        if (registerAllocation.get(arrayVar) == null) {
            handleFloatAssignArrayStatic(arrayVar, size, defaultValue);
        }
        else {
            handleFloatAssignArrayLocal(arrayVar, size, defaultValue);
        }
    }

    private void handleFloatArrayLoad(Array_load instruction) {
        String var = instruction.args().get(0);
        String arrayVar = instruction.args().get(1);
        String index = instruction.args().get(2);
        if (registerAllocation.get(arrayVar) == null) {
            handleFloatArrayLoadStatic(var, arrayVar, index);
        } else {
            handleFloatArrayLoadLocal(var, arrayVar, index);
        }
    }

    private void handleFloatArrayStore(Array_store instruction) {
        String arrayVar = instruction.args().get(0);
        String index = instruction.args().get(1);
        String value = instruction.args().get(2);
        if (registerAllocation.get(arrayVar) == null) {
            handleFloatArrayStoreStatic(arrayVar, index, value);
        } else {
            handleFloatArrayStoreLocal(arrayVar, index, value);
        }
    }
    ...
    private void handleExit(Call instruction) {
        ...
        if (checkIsFloat(instruction.args().get(1))) {
            emit(new li(PRINT_FLOAT_ARG, instruction.args().get(1)));
        }
        else {
    ...
    static class c_eq_s implements MipsInstruction {
        ...
        protected String register_a;
        protected String register_b;
        public c_eq_s(String register_a, String register_b) {
            this.register_a = register_a;
            this.register_b = register_b;
        }
        public String asString() {
            return "c.eq.s" + " " + String.join(", ", this.args());
        }
        @Override
        public List<String> args() {
            return Arrays.asList(this.register_a, this.register_b);
        }
    }

    static class c_lt_s implements MipsInstruction  {
        // stores comparison results in coprocessors conditional bit
        protected String register_a;
        protected String register_b;
        public c_lt_s(String register_a, String register_b) {
            this.register_a = register_a;
            this.register_b = register_b;
        }
        public String asString() {
            return "c.lt.s" + " " + String.join(", ", this.args());
        }
        @Override
        public List<String> args() {
            return Arrays.asList(this.register_a, this.register_b);
    ...
    static class c_le_s implements MipsInstruction  {
        // stores comparison results in coprocessors conditional bit
        protected String register_a;
        protected String register_b;
        public c_le_s(String register_a, String register_b) {
            this.register_a = register_a;
            this.register_b = register_b;
        }
        public String asString() {
            return "c.le.s" + " " + String.join(", ", this.args());
        }
        @Override
        public List<String> args() {
            return Arrays.asList(this.register_a, this.register_b);
    ...
    static class bc1t implements MipsInstruction {
        protected String branch;
        public bc1t(String branch) {
