...
cd $ANTLR_BUILD_DIR
wget http://www.antlr.org/download/antlr-4.9.3-complete.jar
wget http://www.antlr.org/download/antlr4-cpp-runtime-4.9.3-source.zip
wget http://www.antlr.org/download/antlr-runtime-4.9.3.jar
#
# Build c++ Antlr lib
mkdir tmp
cd tmp
...
mv antlr-runtime-4.9.3.jar /usr/local/lib/
echo 'export CLASSPATH=".:/usr/local/lib/antlr-4.9.3-complete.jar:$CLASSPATH"' >> /etc/profile.d/setClassPath.sh
#
# Create antlr link
echo '#!/bin/sh -ue' > /usr/bin/antlr
echo 'java -jar /usr/local/lib/antlr-4.9.3-complete.jar $@' >> /usr/bin/antlr
